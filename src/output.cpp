...
static void close_file(output_t* output) {
    ...
    if (fdata->f) {
        ...
        if(record_duration_sec < fdata->min_rec_length && fdata->delete_short_records) {
            log(LOG_WARNING, "%.4lf MHz too short record: %llu sec, removing\n", fdata->frequency / 1000000.0, (unsigned long long)record_duration_sec);
        ...
        if (channel->outputs[k].type == O_ICECAST) {
            ...
            if (ret != SHOUTERR_SUCCESS || shout_queuelen(icecast->shout) > MAX_SHOUT_QUEUELEN) {
                ...
                if (channel->freqlist[channel->freq_idx].label != NULL) {
                    ...
                    snprintf(description, sizeof(description), "%.4f MHz", channel->freqlist[channel->freq_idx].frequency / 1000000.0);
...
static void print_channel_metric(FILE* f, char const* name, float freq, char* label) {
    fprintf(f, "%s{freq=\"%.4f\"", name, freq / 1000000.0);
    if (label != NULL) {
